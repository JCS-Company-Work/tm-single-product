...
test.describe('TMPC Product Configurator Basket', () => {
	...
	test('Add product to cart via AJAX @critical', async ({ page }) => {
		...
		// Load basket page
		await page.click('.tm-header-cart-link');
	...
	test('Add sample to cart via AJAX @critical', async ({ page }) => {
        ...
        // Check that swatch is in the basket with correct note
        const basketHtml = await page.locator('.mini_cart_item').allInnerTexts();
        expect(basketHtml.some(html => normalize(html).includes(normalize(thirdOptionText)))).toBeTruthy();
	});

	test('Cart item persistence after reload', async ({ page }) => {
		await page.goto(PRODUCT_URL);
		// Select options and add to cart
		await page.click('.obj-top-colour .wapf-swatch label');
		await page.click('.obj-base .wapf-swatch label');
		await page.click('.obj-metal-edge-veneer .wapf-swatch label');
		await page.click('button.single_add_to_cart_button');
		// Reload page
		await page.reload();
		// Open cart and check that custom fields persist
		await page.click('.header-cart, .mini-cart, .cart-contents');
		await expect(page.locator('.cart_item .product-name')).toContainText(['Top', 'Base', 'Metal']);
	});
